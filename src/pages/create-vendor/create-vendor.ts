...
export class CreateVendorPage {
  ...
  ionViewDidLoad() {
    RestaurantSFSConnector.getInstance().addListener("CreateVendorPage", response => {
      this.onExtension(response);
    })
  }

  ionViewWillUnload(){
    RestaurantSFSConnector.getInstance().removeListener("CreateVendorPage");
  }
  ...
  onExtension(response) {
    ...
    if (RestaurantClient.getInstance().doCheckStatusParams(params)) {
      if (cmd == RestaurantCMD.CREATE_VENDOR) {
  ...
  onClickSave(){
    if(this.mVendor.getName().trim() == ''){
      alert("Bạn cần nhập tên chuỗi nhà hàng");
    }else{
      this.mAppModuel.showLoading();
      RestaurantSFSConnector.getInstance().doCreateVendor(this.mVendor);
    }
  }
